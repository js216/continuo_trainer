...
static void note_to_lily(unsigned char note, char *buf, size_t len)
{
   ...
   if (octave >= 4) {
      ...
      int commas = 3 - octave; /* c at octave 3 -> 0 commas; c, -> octave 2 */
...
static void midi_callback(double stamp, const unsigned char *msg,
...
static void restore_from_log(State *s)
{
   if (s->saved_in_name[0]) {
      ...
      if (idx >= 0)
         ...
         out_status("Saved IN device not found: %s", s->saved_in_name);
   }
   if (s->saved_out_name[0]) {
      ...
      if (idx >= 0)
         ...
         out_status("Saved OUT device not found: %s", s->saved_out_name);
   }
   if (s->forward)
...
static void open_midi_in(State *s, int idx)
{
   ...
   rtmidi_in_ignore_types(h, 0, 0, 0); /* don't ignore sysex/timing/sense */
...
static void handle_command(State *s, const char *line)
{
   ...
   if      (sscanf(cmd, "MIDI IN %d",  &n) == 1) { open_midi_in(s, n);  save_log(s);                  }
   else if (sscanf(cmd, "MIDI OUT %d", &n) == 1) { open_midi_out(s, n); save_log(s);                  }
   else if (strcmp(cmd, "MIDI FORWARD ON")  == 0) { s->forward = 1; out_status("Forwarding enabled");  save_log(s); }
   else if (strcmp(cmd, "MIDI FORWARD OFF") == 0) { s->forward = 0; out_status("Forwarding disabled"); save_log(s); }
   else if (strcmp(cmd, "MIDI DEVICES")     == 0) { refresh_devices(s);                               }
   else if (strcmp(cmd, "MIDI TEST")        == 0) { test_midi_out(s);                                 }
   else if (strncmp(cmd, "MIDI", 4)         == 0) { out_status("Unknown MIDI command: %s", cmd);      }
...
int main(void)
{
   ...
   while (s.running) {
      int ret = poll(fds, 2, -1); /* block until stdin or MIDI arrives */
      if (ret < 0)
         break;
      ...
      /* Drain the wake bytes written by midi_callback; the actual MIDI
         processing already happened inside the callback itself. */
