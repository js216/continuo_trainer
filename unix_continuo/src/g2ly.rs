...
mod tests {
    ...
    fn test_ties_across_passing_group() {
        ...
        assert!(
            first.ends_with('~'),
            "group 0 treble chord should be tied; first token={:?}, full={}",
            first,
            treble
        );
        ...
        assert!(
            !tokens[1].ends_with('~'),
            "group 1 should not be tied; token={:?}",
            tokens[1]
        );
    }
    ...
    fn test_no_tie_without_passing() {
        ...
        assert!(
            !treble.contains('~'),
            "no tie between non-passing groups: {}",
            treble
        );
    }
