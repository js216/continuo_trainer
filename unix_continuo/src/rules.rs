...
mod tests {
    ...
    fn make_group(id: usize, bass_actual: &str, realization: &[&str], melody: &[&str]) -> Group {
        Group {
            id,
            passing: false,
            bass: bass_actual.to_string(),
            figures: "0".to_string(),
            melody_notes: melody.iter().map(|s| s.to_string()).collect(),
            bass_actual: bass_actual.to_string(),
            time: 0,
            realization: realization.iter().map(|s| s.to_string()).collect(),
        }
    }

    #[test]
    fn test_pitch_to_semitone() {
        // c' = MIDI 60
        assert_eq!(pitch_to_semitone("c'"), Some(60));
        assert_eq!(pitch_to_semitone("g'"), Some(67));
        assert_eq!(pitch_to_semitone("d'"), Some(62));
        assert_eq!(pitch_to_semitone("fis"), Some(54)); // fis below middle C
    }

    #[test]
    fn test_no_parallel_fifths_detected() {
        // c-g then d-a: parallel fifths
        let w = vec![
            make_group(0, "c", &["g'"], &["g''"]),
            make_group(1, "d", &["a'"], &["a''"]),
        ];
        assert!(rule_no_parallel_fifths(&w).is_err());
    }

    #[test]
    fn test_no_parallel_fifths_ok() {
        // c-g then c-e: not parallel fifths
        let w = vec![
            make_group(0, "c", &["g'"], &["e''"]),
            make_group(1, "c", &["e'"], &["c''"]),
        ];
        assert!(rule_no_parallel_fifths(&w).is_ok());
    }

    #[test]
    fn test_melody_large_leap() {
        let w = vec![
            make_group(0, "c", &["e'"], &["c'"]),
            make_group(1, "g", &["d'"], &["c''"]),
        ];
        // c' to c'': octave leap (12 semitones) > 9
        assert!(rule_melody_no_large_leap(&w).is_err());
    }

    #[test]
    fn test_realization_empty() {
        let w = vec![ make_group(0, "c", &[], &["c'"]) ];
        assert!(rule_realization_not_empty(&w).is_err());
    }

    #[test]
    fn test_parse_group() {
        let line = "GROUP ID:2 BASS:e2 FIGURES:#6 MELODY:b'4 cis''4 BASS_ACTUAL:e TIME:9774 REALIZATION:g'/cis'";
        let g = parse_group(line).unwrap();
        assert_eq!(g.id, 2);
        assert_eq!(g.bass, "e2");
        assert_eq!(g.figures, "#6");
        assert_eq!(g.melody_notes, vec!["b'4", "cis''4"]);
        assert_eq!(g.bass_actual, "e");
        assert_eq!(g.time, 9774);
        assert_eq!(g.realization, vec!["g'", "cis'"]);
        assert!(!g.passing);
    }

    #[test]
    fn test_parse_passing_group() {
        let line = "GROUP ID:1 passing BASS:fis2 FIGURES:0 MELODY:b'4 BASS_ACTUAL:fis TIME:5475 REALIZATION:b'/g'/d'/g";
        let g = parse_group(line).unwrap();
        assert!(g.passing);
        assert_eq!(g.id, 1);
    }
