...
fn px(slot: f64) -> i32 { (MARGIN_X + slot * PX_PER_SLOT).round() as i32 }
fn py(sp:   f64) -> i32 { (MARGIN_Y + sp   * PX_PER_SP  ).round() as i32 }

// Thickness in render.rs is in staff-spaces; convert to integer pixels (min 1).
fn pt(t: f64) -> i32 { (t * PX_PER_SP).round().max(1.0) as i32 }
...
fn stroke_arc(cx: i32, cy: i32, r: i32, a_min: i32, a_max: i32, t: i32, c: (u8, u8, u8)) {
    println!("stroke_arc {} {} {} {} {} {} {} {} {}", cx, cy, r, a_min, a_max, t, c.0, c.1, c.2);
}

fn fill_arc(cx: i32, cy: i32, r: i32, a_min: i32, a_max: i32, c: (u8, u8, u8)) {
    println!("fill_arc {} {} {} {} {} {} {} {}", cx, cy, r, a_min, a_max, c.0, c.1, c.2);
}

#[allow(dead_code)]
fn fill_triangle(x0: i32, y0: i32, x1: i32, y1: i32, x2: i32, y2: i32, c: (u8, u8, u8)) {
    println!("fill_triangle {} {} {} {} {} {} {} {} {}", x0, y0, x1, y1, x2, y2, c.0, c.1, c.2);
}
...
fn py_sp(sp: f64) -> i32 { (sp * PX_PER_SP).round() as i32 }
...
fn draw_flat(x: i32, y: i32) {
    ...
    fill_arc(x, y + py_sp(0.1), (rw as f64 * 0.55) as i32, 270, 90, (255, 255, 255));
}
...
fn parse_f(s: &str) -> f64 { s.parse().unwrap_or(0.0) }
fn parse_i(s: &str) -> i32 { s.parse().unwrap_or(0) }
    ...
    for line in stdin.lock().lines() {
        ...
        match tokens[0] {
            ...
            "TIE" => {
                ...
                stroke_curve(x0, y, ctrl_x0, ctrl_y, ctrl_x1, ctrl_y, x1, y,
                             pt(0.08).max(1), BLACK);
            }
