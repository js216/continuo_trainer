...
void state_read_lesson(const std::string &filename, state *s)
{
    ...
    while (std::getline(file, line))
    {
        ...
        {
            s->lesson_title = line.substr(6);
            s->lesson_title.erase(0, s->lesson_title.find_first_not_of(" \t"));
        }
        else if (line.rfind("description:", 0) == 0)
        {
            s->lesson_description = line.substr(12);
            s->lesson_description.erase(0, s->lesson_description.find_first_not_of(" \t"));
        }
        else if (line.rfind("key:", 0) == 0)
        {
            std::string keystr = line.substr(4);
            keystr.erase(0, keystr.find_first_not_of(" \t"));
            s->key = parse_key(keystr);
        }
    }
    ...
    while (std::getline(file, line))
    {
        ...
        s->chords.push_back(std::move(col));
    }
}
