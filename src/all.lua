...
local function compute_bar_children(
	...
	for _, sp in ipairs(splits) do
		...
		if not seen_hashes[hash] then
			...
			children[#children + 1] = { hash = hash, s = s, e = e, skills = skills_str, content = content }
		end
	end
	return children
end
...
local function write_chunks_js(index)
	...
	f:write("// Generated by make index — do not edit manually\n")
	f:write("window.CHN_INDEX={\n")
	for i, h in ipairs(hashes) do
		...
		f:write(string.format(
			'"%s":{"level":%d,"skills":%s,"children":[%s],"txt":%s}',
			h, c.level, js_str(c.skills or ""), table.concat(child_parts, ","), js_str(c.txt or "")
		))
