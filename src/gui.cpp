...
static const char *alg_tooltip(const char *key)
{
	struct { const char *k; const char *t; } tips[] = {
		...
		{"mastery_points_per_pct", "Points earned per 1% mastery gain"},
		{"power_points_per_pct", "Points earned per 1% power gain"},
		{"bottleneck_thresh", "Speed/evenness factor below which a targeted hint is shown"},
		{"dominance_margin", "How much worse one factor must be vs. others to trigger its hint"},
		{"min_quality", "Quality score below which a 'raise quality' hint appears"},
		{"power_score_frac", "Don't suggest a chunk if its power already exceeds this fraction of mastery"},
		{"overlearn_min", "Fewest consecutive attempts before suggesting a different chunk (when pass rate is perfect)"},
		{"overlearn_max", "Most consecutive attempts before suggesting a different chunk (when pass rate is low)"},
		{"mistake_power_penalty", "How much power drops per failed session (0-1)"},
		{"mastery_decay_half_life", "Days of inactivity for mastery to drop to half"},
		{"skill_order", "Order in which skills are introduced (space-separated).\nNew chunks are suggested in this order."},
		{"weak_ema_thresh", "Pass rate below this marks a lesson as 'needs work'"},
		{"ease_initial", "Starting ease factor for spaced repetition (SM-2)"},
		{"ease_min", "Minimum ease factor (prevents intervals from shrinking too fast)"},
		{"ease_max", "Maximum ease factor"},
		{"ease_pass_delta", "Ease increase after a perfect pass"},
		{"ease_fail_delta", "Ease decrease after a fail"},
		{"ivl_first", "Review interval (days) after first perfect pass"},
		{"ivl_second", "Review interval (days) after second consecutive perfect pass"},
		{"ivl_max", "Maximum review interval in days"},
		{"chunk_mastery_thresh", "Mastery % below which a chunk still needs practice"},
		{"chunk_power_thresh", "Power % below which a chunk still needs practice"},
		{"midnight_time", "Hour (UTC) at which the practice day rolls over (0 = midnight).\nAffects streaks, daily scores, warmup resets, and daily repeat caps."},
		{"srs_allowed_mistakes", "Number of failed groups still counted as a pass for SRS scheduling.\n0 = perfect required, 1 = one mistake tolerated, etc."},
		{"warmup_attempts", "First N plays of a chunk each day skip the SRS penalty on failure.\nMastery still grows normally; only the interval/ease reset is suppressed."},
		{"daily_repeats_min", "Daily play cap for easy chunks (pass rate near 100%).\nChunk won't be suggested after this many plays today."},
		{"daily_repeats_max", "Daily play cap for hard chunks (pass rate near 0%).\nHarder chunks get more daily attempts."},
		{"cross_day_ease_bonus", "Extra ease increase when practicing on a different day than last time.\nRewards distributed practice across days (spacing effect)."},
		{"perf_bpm_step_up", "BPM increase fraction on a perfect karaoke pass (e.g. 0.05 = +5%)"},
		{"perf_bpm_step_down", "BPM decrease fraction on a karaoke fail (e.g. 0.05 = -5%)"},
		{"badge_power_thresh", "Power % needed to earn the practice P badge"},
		{"badge_speed_thresh", "BPM needed to earn the practice S badge.\nAlso the lower bound for automatic karaoke BPM."},
		{"badge_evenness_thresh", "Evenness (0-1) needed to earn the practice E badge"},
		{"perf_badge_power_thresh", "Power % needed to earn the performance P' badge"},
		{"perf_badge_speed_thresh", "BPM needed to earn the performance S' badge.\nAlso the upper bound for automatic karaoke BPM."},
		{"perf_badge_evenness_thresh", "Evenness (0-1) needed to earn the performance E' badge"},
		{"badge_power_bonus", "Points awarded for earning a power badge (P or P')"},
		{"badge_speed_bonus", "Points awarded for earning a speed badge (S or S')"},
		{"badge_evenness_bonus", "Points awarded for earning an evenness badge (E or E')"},
		{"badge_graduate_bonus", "Bonus points for completing all three practice badges"},
		{"badge_mastered_bonus", "Bonus points for completing all six badges"},
		{"perf_fail_accuracy", "Timing accuracy % below which a performance attempt fails"},
		{"perf_fail_timing_tol", "How far off each beat can be before it counts as missed (fraction of beat interval)"},
		{"perf_fail_streak", "Consecutive performance fails before suggesting practice mode"},
		{"rotation_pool_size", "How many chunks to keep in the interleaved practice rotation"},
		{"min_away", "Minimum attempts on other chunks before a rotated-out chunk comes back"},
		{"length_bonus_threshold", "Lessons with this many groups or fewer get no length bonus"},
		{"length_bonus_per_group", "Extra point fraction per group above the threshold (e.g. 0.25 = +25% per group)"},
		{NULL, NULL}
	};
...
static void handle_stats(const char *buf)
{
	...
	p = strstr(buf, "pract_bpm=");
	if (p)
		sscanf(p, "pract_bpm=%f", &state.status.pract_bpm);

	p = strstr(buf, "perf_bpm=");
	if (p)
		sscanf(p, "perf_bpm=%f", &state.status.perf_bpm);
